...
function renderAuth() {
  ...
  bindPortalForm(form, currentForm => {
    ...
    window.location.assign(`account.html?role=${role}`);
...
function accountNavigation(role, view) {
  ...
  return items.map(([key, label]) => `<a class="${view === key ? 'is-active' : ''}" href="account.html?role=${role}&view=${key}" ${view === key ? 'aria-current="page"' : ''}><span aria-hidden="true">${key === 'overview' ? '◇' : key === 'profile' ? '○' : key === 'requests' || key === 'viewings' ? '□' : '▦'}</span>${label}</a>`).join('');
}
...
function renderOwnerProperties(items) {
  return `<div class="owner-property-list">${items.map(item => `<article class="owner-property-row"><a class="owner-property-row__image" href="property.html?id=${item.id}">${item.image ? `<img src="${item.image}" alt="${item.imageAlt || item.title}">` : '<span class="property-card__placeholder"><b>D</b><small>Фото ще не додано</small></span>'}</a><div><div class="owner-property-row__meta"><span>${item.deal === 'rent' ? 'Оренда' : 'Купівля'}</span>${statusBadge('Активне')}</div><h3><a href="property.html?id=${item.id}">${item.title}</a></h3><p>${item.district} · ${propertyTypes[item.type] || 'Нерухомість'}</p><strong>${formatUsd(item.priceUsd)}${item.deal === 'rent' ? '<small> на місяць</small>' : ''}</strong></div><div class="owner-property-row__actions"><a class="button button--primary" href="property-editor.html?id=${item.id}">Редагувати</a><a class="text-link" href="property.html?id=${item.id}">Переглянути <span>→</span></a></div></article>`).join('')}</div>`;
}

function renderAccountContent(role, view, data, forceState) {
  const profile = data[role];
  if (forceState === 'error') return `<div class="portal-state portal-state--error"><span>!</span><h2>Не вдалося відкрити розділ</h2><p>Це демонстраційний error state кабінету.</p><a class="button button--primary" href="account.html?role=${role}&view=${view}">Спробувати ще раз</a></div>`;
...
function renderPropertyEditor() {
  ...
  root.innerHTML = `<nav class="breadcrumbs" aria-label="Навігаційний шлях"><a href="index.html">Головна</a><span>·</span><a href="account.html?role=landlord&view=properties">Кабінет орендодавця</a><span>·</span><span>${editing ? 'Редагування об’єкта' : 'Новий об’єкт'}</span></nav><header class="editor-header"><div><p class="eyebrow"><span></span> Кабінет орендодавця</p><h1>${editing ? 'Редагувати об’єкт' : 'Додати об’єкт'}</h1><p>Заповніть дані, які формуватимуть майбутню картку та детальну сторінку.</p></div><span>Чернетка · frontend</span></header><div class="property-editor-layout"><section class="property-editor-card"><div class="prototype-notice"><b>Демонстраційна форма</b><span>Дані й фотографії не зберігаються та не надсилаються.</span></div>${editorForm(item)}</section>${renderEditorPreview(item)}</div>`;
  const form = root.querySelector('[data-portal-form]');
