...
function initCatalog() {
  ...
  function syncUrl(state) {
    ...
    mapViewLink.href = `map.html${mapParams.size ? `?${mapParams}` : ''}`;
  }
...
function initMapPage() {
  ...
  function syncUrl(state) {
    ...
    Object.entries(state).forEach(([key, value]) => { if (value) next.set(key, value); });
    history.replaceState(null, '', `${location.pathname}${next.size ? `?${next}` : ''}`);
    const catalogHref = `catalog.html${next.size ? `?${next}` : ''}`;
  ...
  function mapCard(item) {
    ...
    return `<article class="map-result-card" data-map-card="${item.id}" tabindex="0" aria-label="${item.title}, ${formatUsd(item.priceUsd)}${unit}">
      <a class="map-result-card__image" href="property.html?id=${item.id}" aria-label="Переглянути об’єкт: ${item.title}">${image}<span>${item.deal === 'sale' ? 'Купівля' : 'Оренда'}</span></a>
      <div class="map-result-card__body">
        ...
        <h3><a href="property.html?id=${item.id}">${item.title}</a></h3>
  ...
  function popupMarkup(item) {
    ...
    return `<span class="map-popup__image">${image}</span><div><small>${item.district || 'Район уточнюється'} · ${item.deal === 'sale' ? 'Купівля' : 'Оренда'}</small><h3>${item.title}</h3><strong>${formatUsd(item.priceUsd)}${unit}</strong><a href="property.html?id=${item.id}">Переглянути об’єкт <span aria-hidden="true">→</span></a></div>`;
  }
...
function initProperty() {
  ...
  if (!item) {
    ...
    root.innerHTML = `<section class="not-found section-shell"><span>404</span><h1>Об’єкт не знайдено</h1><p>Перевірте адресу сторінки або поверніться до каталогу.</p><a class="button button--primary" href="catalog.html">Повернутися до каталогу</a></section>`;
    return;
  }
  ...
  root.innerHTML = `<section class="property-head section-shell">
      <nav class="breadcrumbs" aria-label="Навігаційний шлях"><a href="index.html">Головна</a><span>·</span><a href="catalog.html">Каталог</a><span>·</span><span>${item.title}</span></nav>
    ...
    <section class="section section-shell"><div class="section-heading"><div><p class="eyebrow"><span></span> Ще у добірці</p><h2>Схожі об’єкти</h2></div><a class="text-link" href="catalog.html">До каталогу <span>→</span></a></div><div class="property-grid">${similar.map(renderPropertyCard).join('')}</div></section>
